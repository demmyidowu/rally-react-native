 ...
 */
export const searchPlaces = onCall(async (request): Promise<{ predictions: PlacePrediction[] }> => {
 ...
 */
export const getPlaceDetails = onCall(async (request): Promise<PlaceDetails> => {
    // Verify user is authenticated
    if (!request.auth) {
